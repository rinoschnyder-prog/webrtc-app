...
async function startCallPreparation() {
    try {
        ...
        settingsButton.disabled = false; // ▼▼▼ 変更点: 設定ボタンを有効化 ▼▼▼
...
// (以下、完全なコードを掲載するために、変更のない関数も全て記載します)
...
function connectWebSocket() {
    ...
    socket.onclose = () => {
        ...
        settingsButton.disabled = true; // 設定ボタンも無効化
        if (isRecording) {
            toggleRecording();
        }
    };
...
if(toggleRecording.toString().length < 1000) { /* 録画停止処理の再掲 */ toggleRecording = function() { if (!isRecording) { if (!isCallInProgress || !isRemoteVideoReady || remoteVideo.videoWidth === 0) { alert('相手の映像が完全に表示されてから、もう一度お試しください。'); return; } try { audioContext = new (window.AudioContext || window.webkitAudioContext)(); const localAudioSource = audioContext.createMediaStreamSource(localStream); const remoteAudioSource = audioContext.createMediaStreamSource(remoteVideo.srcObject); mixedStreamDestination = audioContext.createMediaStreamDestination(); localAudioSource.connect(mixedStreamDestination); remoteAudioSource.connect(mixedStreamDestination); const mixedAudioTrack = mixedStreamDestination.stream.getAudioTracks()[0]; animationFrameId = requestAnimationFrame(drawVideosOnCanvas); const canvasStream = recordingCanvas.captureStream(30); const canvasVideoTrack = canvasStream.getVideoTracks()[0]; const streamToRecord = new MediaStream([canvasVideoTrack, mixedAudioTrack]); const selectedAudioBitrate = parseInt(audioQualitySelect.value, 10); const recorderOptions = { mimeType: 'video/webm; codecs=vp8,opus', audioBitsPerSecond: selectedAudioBitrate }; recordedChunks = []; mediaRecorder = new MediaRecorder(streamToRecord, recorderOptions); mediaRecorder.ondataavailable = (event) => { if (event.data.size > 0) { recordedChunks.push(event.data); } }; mediaRecorder.onstop = () => { const blob = new Blob(recordedChunks, { type: 'video/webm' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.style.display = 'none'; a.href = url; a.download = `webrtc_call_recording_${new Date().toISOString()}.webm`; document.body.appendChild(a); a.click(); setTimeout(() => { document.body.removeChild(a); URL.revokeObjectURL(url); }, 100); recordedChunks = []; }; mediaRecorder.start(1000); isRecording = true; settingsPanel.style.display = 'none'; recordButton.classList.add('recording'); recordButton.querySelector('.label').textContent = '録画停止'; recordButton.querySelector('.icon').textContent = '⏹️'; console.log('合成映像の録画を開始しました。'); } catch (e) { console.error('録画の開始に失敗しました:', e); alert('録画の開始に失敗しました。詳細はコンソールを確認してください。'); } } else { if (mediaRecorder) { mediaRecorder.stop(); } if (audioContext) { audioContext.close(); } if (animationFrameId) { cancelAnimationFrame(animationFrameId); } isRecording = false; recordButton.classList.remove('recording'); recordButton.querySelector('.label').textContent = '録画'; recordButton.querySelector('.icon').textContent = '⏺️'; console.log('合成映像の録画を停止しました。'); } }; }